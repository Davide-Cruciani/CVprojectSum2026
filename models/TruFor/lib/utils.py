...
def get_model(config):
    if config.MODEL.NAME == 'detconfcmx':
        from .models.cmx.builder_np_conf import EncoderDecoder as detconfcmx
...
def get_criterion(config):
    ...
    for (l,_,criterion) in losses:
        ...
        # Training the Confidence
            ...
            if criterion == 'mse':
                from models.TruFor.lib.core.criterion_conf import MSE
        ...
        # Training the Detector
            ...
            if detection is not None and not detection == 'none':
                if criterion == 'cross_entropy':
                    from models.TruFor.lib.core.criterion_det import CrossEntropy
...
def get_optimizer(model, config):
    if 'cmx' in config.MODEL.NAME:
        from models.TruFor.lib.models.cmx.init_func import group_weight
        params_list = []
